...
# ==============================================================================
# Modifications copyright (C) 2020 FZI Forschungszentrum Informatik
# - Ported tensorflow code to version 2.0
# ==============================================================================
...
def encode_initial_conditions(init_pos, init_hd, place_cell_ensembles,
                              head_direction_ensembles, name=None):
    initial_conds = []
    for ens in place_cell_ensembles:
        initial_conds.append(
            tf.squeeze(ens.get_init(
                init_pos[:, tf.newaxis, :]), axis=1, name=name)
        )

    for ens in head_direction_ensembles:
        initial_conds.append(
            tf.squeeze(ens.get_init(init_hd[:, tf.newaxis, :]), axis=1, name=name)
        )
...
def get_scores_and_plot(scorer,
    ...
    xy = tf.reshape(data_abs_xy, (-1, data_abs_xy.shape[-1]))
    act = tf.reshape(activations, (-1, activations.shape[-1]))
    n_units = act.shape[1]
