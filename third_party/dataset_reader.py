...
class DataReader(object):
    ...
    def __init__(
        ...
        with tf.device('/cpu'):
            ...
            record_dataset = tf.data.TFRecordDataset(file_names)
            # reader = tf.TFRecordReader()

            read_ops = [
                # self._make_read_op(reader, filename_queue) for _ in range(num_threads)
                self._read_record_example(record_dataset) for _ in range(num_threads)
            ]
    ...
    def _read_record_example(self, record_dataset):
        feature_map = {
            'init_pos':
